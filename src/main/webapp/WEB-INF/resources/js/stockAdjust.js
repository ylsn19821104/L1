...
$(function () {
    bindHandlers();
    bindT2Handlers();
    var t1Url = 'stockAdjust';
    var t2Url = 'stockAdjustDtl'
    ...
    function toEdit() {
        ...
        if (rows) {
            if (rows.length > 1) {
                ...
                $('#create_time').datebox('setValue',new Date($.currentItem.create_time).format('yyyy-MM-dd'));
                setEditable($.currentItem.stat != 1);//已完成状态不可编辑
                t2Query();
    ...
    function initSkuCombo() {
        ...
        if (!rows || rows.length < 1) {
            $.ajax({
                ...
                type: 'get',
                data: {billId: $.currentItem.id,tableName:'tb_stockadjustdtl'},
                dataType: 'json',
                async: false
            }).success(function (ret) {
                $('#skuId').combobox({
